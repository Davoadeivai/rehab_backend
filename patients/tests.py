...
class PatientViewSetTests(APITestCase):
    ...
    def test_create_patient(self):
        ...
        self.assertEqual(
            Patient.objects.get(
                file_number='TEST002').national_code,
            '0987654321'
        )
    ...
    def test_update_patient(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Patient.objects.get(pk=self.patient.pk).first_name, 'تست‌شده'
        )
...
class PrescriptionViewSetTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.prescription = Prescription.objects.create(
            ...
            total_prescribed=Decimal('1500.00'),
...
class PatientValidationTests(TestCase):
    ...
    def test_invalid_national_code_short(self):
        ...
        with self.assertRaises(
            ValidationError,
            msg="کد ملی باید دقیقا ۱۰ رقم باشد."
        ):
            patient.save()
    ...
    def test_invalid_national_code_non_numeric(self):
        ...
        with self.assertRaises(
            ValidationError,
            msg="کد ملی باید فقط شامل اعداد باشد."
        ):
            patient.save()
    ...
    def test_national_code_empty(self):
        with self.assertRaises(
            ...
            self.create_patient(national_code='').full_clean()
    ...
    def test_phone_number_none(self):
        # Validation in save() is `if self.phone_number:`, so None should skip
        # it.
    ...
    def test_phone_number_empty(self):
        # Validation in save() is `if self.phone_number:`, so "" should skip
        # it.
    ...
    def test_valid_dates_withdrawal_none(self):
        ...
        try:
            ...
            self.fail(
                "Valid dates with withdrawal_date=None raised ValidationError.")
    ...
    def test_valid_dates_admission_and_withdrawal_none(self):
        ...
        try:
            ...
            self.fail(
                "Valid dates with admission_date=None and withdrawal_date=None raised ValidationError.")
...
class PrescriptionValidationTests(TestCase):
    def setUp(self):
        ...
        DrugInventory.objects.filter(
            medication_type=self.medication_type).delete()
    ...
    def create_prescription(self, **kwargs):
        ...
        if 'medication_type' not in kwargs:
            data['medication_type'] = self.medication_type
...
class MedicationDistributionValidationTests(TestCase):
    def setUp(self):
        ...
        # ساخت سهمیه دارو برای بیمار
        DrugQuota.objects.filter(patient=self.patient).delete()
        self.quota = DrugQuota.objects.create(
            patient=self.patient,
            total_quota=200.0,  # مقدار کافی برای تست‌ها
            remaining_quota=200.0
        )
        self.medication_type = MedicationType.objects.create(
            name='متادون',
            unit='میلی‌گرم'
        ...
        self.valid_distribution_data = {
            ...
            # Assuming this is calculated or not strictly validated here
    ...
    def test_invalid_single_distribution_exceeds_total_prescribed(self):
        distribution = self.create_distribution(
            amount=Decimal('101.00'))  # Prescription total is 100
    ...
    def test_invalid_cumulative_distribution_exceeds_total_prescribed(self):
        dist1 = self.create_distribution(amount=Decimal(
            '60.00'), distribution_date=date(2023, 1, 1))
        dist1.save()  # This one is valid and reduces stock

        self.inventory.refresh_from_db()

        dist2 = self.create_distribution(
            amount=Decimal('50.00'), distribution_date=date(
                2023, 1, 2))  # 60 + 50 = 110 > 100
        with self.assertRaisesMessage(ValidationError, "بیشتر از مقدار کل تجویز شده"):
            dist2.save()
    ...
    def test_update_distribution_exceeds_total_prescribed(self):
        ...
        # First, let's make another small distribution to make the test more
        # interesting
        dist2 = self.create_distribution(amount=Decimal(
            '10.00'), distribution_date=date(2023, 1, 3))
        dist2.save()  # Total distributed = 20. Remaining allowed = 80.

        # Try to update first from 10 to 90. 90 (new) + 10 (dist2) = 100. This
        # should be fine.
        distribution.amount = Decimal('90.00')
        try:
            # This save is an update, inventory logic in save() is skipped for
            # updates.
            distribution.save()
        except ValidationError:
            self.fail(
                "Updating distribution to a valid cumulative amount failed.")

        # 91 (new) + 10 (dist2) = 101 > 100.
        distribution.amount = Decimal('91.00')
        with self.assertRaisesMessage(ValidationError, "بیشتر از مقدار کل تجویز شده"):
            distribution.save()
    ...
    def test_valid_distribution_date(self):
        distribution = self.create_distribution(
            distribution_date=date(
                2023, 1, 5))  # Prescription 1/1 to 1/10
        try:
            distribution.save()
        except ValidationError:
    ...
    def test_invalid_distribution_date_before_prescription_start(self):
        distribution = self.create_distribution(
            distribution_date=date(2022, 12, 31))
    ...
    def test_invalid_distribution_date_after_prescription_end(self):
        distribution = self.create_distribution(
            distribution_date=date(2023, 1, 11))
    ...
    def test_distribution_sufficient_inventory(self):
        ...
        try:
            ...
            self.fail(
                "Distribution with sufficient inventory raised ValidationError.")
    ...
    def test_distribution_insufficient_inventory(self):
        ...
        # The error message comes from the model's save method
        # (MedicationDistribution)
...
class ManagementCommandTests(TestCase):
    def setUp(self):
        ...
        self.med_type = MedicationType.objects.create(
            name="Methadone Syrup", unit="ml")
        DrugInventory.objects.filter(medication_type=self.med_type).delete()
        self.inventory = DrugInventory.objects.create(
            medication_type=self.med_type, current_stock=Decimal('1000.00'))

        self.prescription = Prescription.objects.create(
            patient=self.patient, medication_type=self.med_type,
            # 50 * 10
            daily_dose=Decimal('50'), treatment_duration=10, total_prescribed=Decimal('500'),
            start_date=date(2023, 1, 1), end_date=date(2023, 1, 10)
        )

    def test_scan_patients_command(self):
        # Valid patient (already created in general setUp if needed, or create
        # one here)
        ...
        p_invalid_date = Patient.objects.create(
            ...
            marital_status='single', education='high_school', treatment_type='detox', usage_duration="6m",
            # Admission < Birth
            date_birth=date(2023, 2, 1), admission_date=date(2023, 1, 1)
        )

        out = StringIO()
        # Count self.patient as well for total scanned
        # Should be 5 if self.patient is unique
        total_patients_including_cmdtest01 = Patient.objects.count()

        call_command('scan_patients', stdout=out)
        output = out.getvalue()

        self.assertIn(
            f"ERROR: Patient file_number {
                p_invalid_nc.file_number}",
            output)
        self.assertIn("کد ملی باید دقیقا ۱۰ رقم باشد.", output)
        self.assertIn(
            f"ERROR: Patient file_number {
                p_invalid_phone.file_number}",
            output)
        self.assertIn("شماره تلفن باید ۱۱ رقم باشد.", output)
        self.assertIn(
            f"ERROR: Patient file_number {
                p_invalid_date.file_number}",
            output)
        self.assertIn("تاریخ تولد باید قبل از تاریخ پذیرش باشد.", output)

        self.assertIn(
            f"Scan complete. {total_patients_including_cmdtest01} patients scanned, 3 errors found.",
            output)
    ...
    def test_scan_prescriptions_command(self):
        ...
        pr_invalid_date = Prescription.objects.create(
            ...
            # End < Start
            start_date=date(2023, 2, 10), end_date=date(2023, 2, 1)
        )
        pr_invalid_total = Prescription.objects.create(
            patient=self.patient, medication_type=self.med_type,
            # Expected 300
            daily_dose=Decimal('30'), treatment_duration=10, total_prescribed=Decimal('301'),
        ...
        self.assertIn(
            "مقدار کل تجویز شده (301) با مقدار محاسبه شده (300) مغایرت دارد.",
            output)

        self.assertIn(
            f"Scan complete. {total_prescriptions} prescriptions scanned, 2 errors found.",
            output)

    def test_scan_medication_distributions_command(self):
        # Valid distribution
        MedicationDistribution.objects.create(
            prescription=self.prescription, distribution_date=date(
                2023, 1, 2), amount=Decimal('50'))

        # Invalid: Amount > total_prescribed for the prescription (total_prescribed is 500)
        # We need a new prescription for this or ensure this is the only
        # distribution to clearly exceed
        prescription_for_big_dist = Prescription.objects.create(
            patient=self.patient, medication_type=self.med_type,
            # Max 50
            daily_dose=Decimal('10'), treatment_duration=5, total_prescribed=Decimal('50'),
            start_date=date(2023, 2, 1), end_date=date(2023, 2, 5)
        )
        md_invalid_amount = MedicationDistribution.objects.create(
            prescription=prescription_for_big_dist, distribution_date=date(
                2023, 2, 1),
        ...
        md_invalid_date = MedicationDistribution.objects.create(
            prescription=self.prescription, distribution_date=date(
                2023, 1, 15),  # Prescription ends 2023,1,10
            amount=Decimal('20')
        )
        ...
        # The scan command re-implements checks, but not inventory
        # side-effects.
        ...
        self.assertIn(
            f"ERROR: MedicationDistribution ID {md_invalid_amount.id}:",
            output
        )
        self.assertIn(
            f"مجموع مقادیر توزیع شده برای نسخه ID {prescription_for_big_dist.id} "
            f"({md_invalid_amount.amount}) بیشتر از مقدار کل تجویز شده "
            f"({prescription_for_big_dist.total_prescribed}) است.",
            output
        )
        self.assertIn(
            f"ERROR: MedicationDistribution ID {md_invalid_date.id}:",
            output
        )
        self.assertIn(
            f"تاریخ توزیع ({md_invalid_date.distribution_date.strftime('%Y-%m-%d') if isinstance(md_invalid_date.distribution_date, date) else md_invalid_date.distribution_date}) "
            f"نمی‌تواند بعد از تاریخ پایان نسخه ({self.prescription.end_date.strftime('%Y-%m-%d') if isinstance(self.prescription.end_date, date) else self.prescription.end_date}) باشد",
            output
        )
        self.assertIn(
            f"Scan complete. {total_distributions} distributions scanned, 2 errors found.",
            output
        )
        # Try to update the distribution. Current save logic skips inventory check for updates.
        # تعریف distribution برای جلوگیری از خطای F821
        distribution = MedicationDistribution.objects.filter(prescription=self.prescription).first()
        distribution.notes = "Updated notes."
        distribution.amount = Decimal('25.00')
        try:
            distribution.save()  # Should save without inventory error.
        except ValidationError as e:
            if "موجودی" in str(e):
                self.fail(
                    "Updating distribution unexpectedly failed due to inventory check."
                )
            self.fail(
                f"Updating distribution failed with unexpected ValidationError: {e}"
            )
        self.inventory.refresh_from_db()
        self.assertEqual(
            self.inventory.current_stock,
    ...
    def test_valid_total_prescribed_zero_dose(self):
        ...
        try:
            ...
            self.fail(
                "Valid total_prescribed with zero dose raised ValidationError.")
    ...
    def test_valid_total_prescribed_zero_duration(self):
        ...
        try:
            ...
            self.fail(
                "Valid total_prescribed with zero duration raised ValidationError.")
    ...
    def test_total_prescribed_recalculation(self):
        ...
        # This test actually verifies that if total_prescribed is wrong, it
        # errors.
        prescription = self.create_prescription(
        ...
        expected_msg_part = f"مقدار کل تجویز شده ({
            self.valid_prescription_data['total_prescribed']}) با مقدار محاسبه شده (150.00) مغایرت دارد."
        with self.assertRaisesMessage(ValidationError, expected_msg_part):
            prescription.save()
