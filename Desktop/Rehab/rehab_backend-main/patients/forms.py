...
class PatientForm(forms.ModelForm):
	...
	admission_date = JalaliDateField(label='تاریخ تشکیل پرونده')
	treatment_withdrawal_date = JalaliDateField(label='تاریخ خروج از درمان', required=False)
    ...
    class Meta:
        ...
        widgets = {
			...
			'addiction_start_age': forms.NumberInput(attrs={'class': 'form-control', 'min': 1, 'max': 120}),
			'file_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'مثلاً: 00001'}),
        }
    ...
    def __init__(self, *args, **kwargs):
		...
		# مقداردهی اولیه تاریخ تشکیل پرونده
    ...
    def clean(self):
        ...
        if withdrawal_date and admission_date and withdrawal_date < admission_date:
			raise ValidationError('تاریخ خروج از درمان نمی‌تواند قبل از تاریخ تشکیل پرونده باشد')

        if admission_date and date_birth and admission_date < date_birth:
			raise ValidationError('تاریخ تشکیل پرونده نمی‌تواند قبل از تاریخ تولد باشد')
...
class PrescriptionItemForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
		...
		# Make duration optional; will fallback to prescription duration
...
class PrescriptionForm(forms.ModelForm):
    start_date = JalaliDateField(
        label='تاریخ شروع',
		help_text='تاریخ شروع مصرف دارو'
    )
    end_date = JalaliDateField(
        label='تاریخ پایان',
		help_text='تاریخ پایان مصرف دارو',
        required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
			...
			self.fields['treatment_duration'].label = 'مدت مصرف (روز)'
	...
	def save(self, commit=True):
		...
		# محاسبه خودکار تاریخ پایان اگر خالی باشد
		try:
			import jdatetime
			from datetime import timedelta
			if instance.start_date and instance.treatment_duration and not instance.end_date:
				instance.end_date = instance.start_date + timedelta(days=int(instance.treatment_duration))
		except Exception:
			pass
		if commit:
			instance.save()
