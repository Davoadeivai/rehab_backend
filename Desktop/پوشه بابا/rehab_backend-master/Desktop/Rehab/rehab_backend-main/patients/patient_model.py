...
class Patient(models.Model):
    ...
    admission_date = jmodels.jDateField("تاریخ تشکیل پرونده", null=True, blank=True, help_text="مثال: ۱۴۰۲/۰۱/۰۱")
    treatment_withdrawal_date = jmodels.jDateField("تاریخ خروج از درمان", null=True, blank=True, help_text="مثال: ۱۴۰۲/۰۱/۰۱")
    addiction_start_age = models.PositiveIntegerField("سن شروع اعتیاد", null=True, blank=True, validators=[MinValueValidator(1), MaxValueValidator(120)])
    ...
    def save(self, *args, **kwargs):
        ...
        if self.date_birth and self.admission_date:
            if self.date_birth >= self.admission_date:
                raise ValidationError("تاریخ تولد باید قبل از تاریخ تشکیل پرونده باشد.")

        if self.admission_date and self.treatment_withdrawal_date:
            if self.admission_date >= self.treatment_withdrawal_date:
                raise ValidationError("تاریخ تشکیل پرونده باید قبل از تاریخ خروج از درمان باشد.")
        ...
        if self.treatment_start_date and self.admission_date:
            if self.treatment_start_date < self.admission_date:
                raise ValidationError("تاریخ شروع درمان باید بعد از تاریخ تشکیل پرونده باشد.")
