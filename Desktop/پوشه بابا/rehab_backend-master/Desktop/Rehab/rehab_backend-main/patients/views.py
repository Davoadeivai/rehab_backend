...
from .forms import (JalaliDateField, PatientForm, PaymentForm, PrescriptionForm, MedicationDistributionForm,
                    UserProfileForm, UserSettingsForm, ContactForm, SupportForm, FeedbackForm, MedicationForm, MedicationAdministrationForm, ServiceTransactionForm, PrescriptionItemFormSet)
...
def patient_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # شماره پرونده باید از فرم تامین شود؛ تولید خودکار حذف شد
            print(f'Using provided file_number: {patient.file_number}', file=sys.stderr)
            try:
                patient.save()
...
def prescription_create(request):
    """
    ایجاد نسخه جدید برای بیمار با قابلیت ثبت چندین دارو
    """
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            # Save the formset with the prescription instance
            ...
            messages.success(
                request, 
                'نسخه با موفقیت ثبت شد. لطفاً اطلاعات پرداخت را تکمیل کنید.',
                'success'
            )
        ...
        formset = PrescriptionItemFormSet(prefix='drugs')
